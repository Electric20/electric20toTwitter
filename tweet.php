...
$oauth_token,$oauth_token_secret);
			...
			if ($i<3) {
				...
				$text='Today ('.$printDate.'), you were among the 3 neighbours that use the most electricity.';
			...
			if ($i>($count-4)) {
				...
				$text='Today ('.$printDate.'), you were among the 3 neighbours that use the least electricity. Well done.';
...
function getCompareToNeighbourText($name,$usage,$av) {
	...
	if ($usage<$av) {
		$text = 'Today, you have used '.round($usage,3). ' kwh of electricity. You used '.round($diff,2).'% less than your neighbours.';
	}
	if ($usage>$av) {
		$text =  'Today, you have used '.round($usage,3). ' kwh of electricity. You used '.round($diff,2).'% more than your neighbours.';
	}
	return $text;
}
...
function getCompareToYesterdayText($name,$usage,$usageY) {
	...
	if ($usage<$usageY) {			
		$text = 'You used '.round($diffToY,2).'% less than yesterday. '.getCongrats();  
	}
	if ($usage>$usageY) {
		$text = 'You used '.round($diffToY,2).'% more than yesterday. '.getTip();
	}
	return $text;
}
