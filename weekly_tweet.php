...
//TODO: change to get weekly array
...
$count = $test->count;
$test = $obj->prev7; 
$averageL = $test->average;
$countL = $test->count;
$i = 0;

foreach ($obj->last7 as $thisweek) {
       //echo"{$today->hubId}\n";
	
	$hubId = $thisweek->hubId;
	$name = $thisweek->description;
	$usage = $thisweek->total;
	...
	echo "hubId: '.$hubId.' , name: '.$name.', usageThisWeek: '.$usage.', averageThisWeek: '.$av.', hubIdL: '.$hubIdY.', usageLastWeek: '.$usageL.', averageLastWeek '.$averageL \n";  

	if ($hubId > 0) {
		$text = getCompareToNeighbourText($name,$usage,$av);
		echo "++++TWEET1++++: '.$text\n";
		$text2 = getCompareToLastWeekText($name,$usage,$usageY);				echo "++++TWEET2++++: '.$text \n";
...
$oauth_token,$oauth_token_secret);
			...
			if ($i<3) {
				...
				$text=$printDate.': Last week, you were among the 3 neighbours that use the most electricity.';
			...
			if ($i>($count-4)) {
				...
				$text=$printDate.': Last week, you were among the 3 neighbours that use the least electricity. Well done.';
...
function getCompareToNeighbourText($name,$usage,$av) {
	...
	if ($usage<$av) {
		$text = 'Last week, you used '.round($usage,3). ' kwh of electricity. You used '.round($diff).'% less than your neighbours.';
	}
	if ($usage>$av) {
		$text =  'Last week, you used '.round($usage,3). ' kwh of electricity. You used '.round($diff).'% more than your neighbours.';
	}
	return $text;
}

function getCompareToLastWeekText($name,$usage,$usageY) {
	...
	if ($usage<$usageY) {
		$text = 'Last week, you used '.round($diffToY).'% less than the week before. '.getCongrats();  
	}
	if ($usage>$usageY) {
		$text = 'Last week, you used '.round($diffToY).'% more than the week before. '.getTip();
	}
